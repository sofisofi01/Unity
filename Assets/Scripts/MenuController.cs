...
public class MenuController : MonoBehaviour
{
    ...
    private void LoadParams()
    {
        ...
        if (PlayerPrefs.HasKey("SoundVol"))
        {
            ...
            soundMixer.SetFloat("MasterVolume", Mathf.Log10(soundVol)*  20);
        }
        else
        {
            // Default sound volume (likely 1 or 0.5)
            float defaultSoundVol = 1f; // Replace with desired default
            soundMixer.SetFloat("MasterVolume", Mathf.Log10(defaultSoundVol) *20);
        }
        ...
        if (PlayerPrefs.HasKey("MusicVol"))
        {
            ...
            musicMixer.SetFloat("MasterVolume", Mathf.Log10(musicVol) * 20);
        }
        else
        {
            // Default music volume (likely 1 or 0.5)
            float defaultMusicVol = 1f; // Replace with desired default
            musicMixer.SetFloat("MasterVolume", Mathf.Log10(defaultMusicVol) * 20);
        }
    }
